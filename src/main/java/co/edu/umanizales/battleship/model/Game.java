...
public class Game {
    ...
    public void createBoards(User player1, User player2, ListDE shipList)
    {
        if (shipList.getSize() <=9)
        {
            this.boardPlayer1 = new Board((byte)10, (byte)10, player1, shipList);
            this.boardPlayer1 = new Board((byte)10, (byte)10, player1, shipList.cloneList());
        }
        else if (shipList.getSize() >=10 && shipList.getSize() <=20)
        {
            this.boardPlayer1 = new Board((byte)20, (byte)20, player1, shipList);
            this.boardPlayer1 = new Board((byte)20, (byte)20, player1, shipList.cloneList());
        }

        else
        {
            this.boardPlayer1 = new Board((byte)30, (byte)30, player1, shipList);
            this.boardPlayer1 = new Board((byte)30, (byte)30, player1, shipList.cloneList());
        }
    }

    public String validateShot(int x, int y, User player)
    {
        return null; // si player es igual a tablero1 validar en tablero 2
    }
